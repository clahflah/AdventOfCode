...
minimum_cubes_hash = {"red" => 1 , "green" => 1,"blue" => 1}
...
inventory_hash.each do |key, value|
  value.each do |handful|
    handful.each do |color, amount|
      if minimum_cubes_hash[color] == 1 || minimum_cubes_hash[color] < amount
        minimum_cubes_hash[color] = amount
      end
    end
  end
  puts minimum_cubes_hash.length
  puts product_arrays.length
  puts '^products array length'
  minimum_cubes_hash.each do |color, amount|
...
puts product_arrays.length
sum_challenge_2 = product_arrays.sum
puts sum_challenge_2
