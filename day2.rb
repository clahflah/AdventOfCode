...
input.each_with_index do |game, index|
  game = game.gsub("Game #{index + 1}: ", "")
  game = game.split(';')
  inventory_hash[index + 1] = game
end
inventory_hash. each do |key, value|
  inventory_hash[key] = value.map do |game|
    result_hash = {}
    game.split(',').each do |item|
    quantity, color = item.split
    result_hash[color] = quantity.to_i
  end
  result_hash
end
end
...
sum = 0
# inventory_hash.each do |key, value|
#   puts "LOOP 1---- KEY = #{key} oo Value = #{value}"
#   number_of_correct_handfuls = 0 #used to check if all turns in round are valid
#   necesarry_handful_length = 0 #used to check if all options in a turn are valid
#   value.each do |handful|
#     necesarry_handful_length = handful.length
#     puts "===== NEW HANDFUL ====="
#     puts "LOOP 2#{handful}"
#     game_turns = 0 #used to check how many turns are actually valid
#     handful.each do |color, number|
#       puts "#{color} oo #{number}"
#       inventory.each do |inventory_color, inventory_number|
#         puts "INVENTORY COLOR #{inventory_color}"
#         puts "INVENTORY NUMBER #{inventory_number}"
#         if inventory_color.to_s == color && inventory_number >= number
#           game_turns += 1
#           puts "game turns = #{game_turns} WAS UPDATED"
#         else
#           puts "Handful not valid GAME TURNS = #{game_turns}"
#         end
#       end
#       puts "Handful Length = #{handful.length}"
#       puts "Needed handful length = #{necesarry_handful_length}"
#       puts "NUMBER OF CORRECT HANDFULS #{number_of_correct_handfuls}"
#     end
#     if game_turns == necesarry_handful_length
#       puts "successful game turns matches handful length"
#       number_of_correct_handfuls += 1
#     end

#   end
#   if number_of_correct_handfuls = necesarry_handful_length
#     puts "NUMBER OF CORRECT HANDFULS NEEDED #{necesarry_handful_length}"
#     puts "NUMBER OF CORRECT HANDFULS #{number_of_correct_handfuls}"
#     sum += key
#     puts "SUM WAS ADDED"
#   end
...
inventory_hash.each do |key, value|
  number_of_correct_handfuls = 0 # used to check if all turns in the round are valid

  value.each do |handful|
    game_turns = handful.count do |color, number|
      inventory.any? { |inventory_color, inventory_number| inventory_color.to_s == color && inventory_number >= number }
    end

    if game_turns == handful.length
      number_of_correct_handfuls += 1
    end
  end

  if number_of_correct_handfuls == value.length
    sum += key
  end
end
puts sum

minimum_cubes_hash = {"red": nil, "green": nil,"blue": nil}

product = 1
inventory_hash.each do |key, value|
  counter_checker = 0
  counter_handful_checker = 0
  value.each do |handful|
    counter = 0
    counter_handful_checker = handful.length
    handful.each do |color, amount|
      if minimum_cubes_hash["red"] == nil || minimum_cubes_hash["red"] > amount && minimum_cubes_hash.key("red")
        counter += 1
      elsif minimum_cubes_hash["green"] == nil || minimum_cubes_hash["green"] > handful && minimum_cubes_hash.key("green")
        counter += 1
      elsif minimum_cubes_hash["blue"] == nil || minimum_cubes_hash["blue"] > handful && minimum_cubes_hash.key("blue")
        counter += 1
      end
    end
    counter_checker = counter
  end
  if counter_checker == counter_handful_checker
    product *= key
    puts "PRODUCT MULTIPLIED BY #{key}"
  end
  product
end
puts product
