...
code ="tmnoneightdlrtninenines847bhldgkpdm
threeight"

codeArray = code.split(' ');
sum = 0

def convert_spelled_out(word)
  case word.downcase
  when "one" then 1
  when "two" then 2
  when "three" then 3
...
codeArray.each do |message|
  digits = message.scan(/(?:one|two|three|four|five|six|seven|eight|nine)|[1-9]/)
  digits.map! do |part|
    if part.to_s.match?(/\d+/)
      part.to_i
    else
      convert_spelled_out(part) || part.scan(/\d/).first.to_i
    end
  end
  first_digit = digits.find { |d| d.is_a?(Integer) }
  last_digit = digits.reverse.find { |d| d.is_a?(Integer)}
  number_needed = "#{first_digit}#{last_digit}"
  sum += number_needed.to_i
  puts "Message: #{message}"
  puts "Digits: #{digits}"
  puts "Number needed: #{number_needed}"
